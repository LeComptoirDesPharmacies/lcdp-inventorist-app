from api.consume.gen.auth.models.any_authentication_credential import AnyAuthenticationCredential
...
def __create_api_key(email, password):
    ...
    token = manage_api_key_api.create_api_key(
        {'name': API_KEY_NAME, 'owner_id': user.id},
    ...
    if api_key and api_key_id:
        try:
            ...
            manage_api_key_api.delete_api_key(
                _request_auth=manage_api_key_api.api_client.create_auth_settings("apiKeyAuth", api_key),
